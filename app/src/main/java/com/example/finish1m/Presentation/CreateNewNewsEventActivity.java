...
import com.example.finish1m.Domain.UseCases.CreateNewNewsEventUseCase;
...
import com.example.finish1m.databinding.ActivityCreateNewNewsEventBinding;

import java.util.ArrayList;

public class CreateNewNewsEventActivity extends AppCompatActivity {


    private ActivityCreateNewNewsEventBinding binding;

    private EventRepositoryImpl eventRepository;
    private ChatRepositoryImpl chatRepository;
    private ImageRepositoryImpl imageRepository;
    private CreateNewNewsEventUseCase createNewNewsEventUseCase;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityCreateNewNewsEventBinding.inflate(getLayoutInflater());
            ...
            public void onClick(View view) {
                ...
                if(!TextUtils.isEmpty(title)){
                    if(!TextUtils.isEmpty(title)){
                        ...
                        Event e = new Event(eventRepository.getNewId(), Event.NEWS, title, message, c.getId(), null, null);
                        createNewNewsEventUseCase = new CreateNewNewsEventUseCase(eventRepository, chatRepository, imageRepository, e, c, images, new OnSetDataListener() {
                            @Override
                            public void onSetData() {
                                Toast.makeText(CreateNewNewsEventActivity.this, R.string.event_create_success, Toast.LENGTH_SHORT).show();
                                finish();
                            }

                            @Override
                            public void onFailed() {
                                Toast.makeText(CreateNewNewsEventActivity.this, R.string.error, Toast.LENGTH_SHORT).show();
                                finish();
                            }

                            @Override
                            public void onCanceled() {
                                Toast.makeText(CreateNewNewsEventActivity.this, R.string.access_denied, Toast.LENGTH_SHORT).show();
                                finish();
                            }
                        });
                        createNewNewsEventUseCase.execute();
                    }
                    else {
